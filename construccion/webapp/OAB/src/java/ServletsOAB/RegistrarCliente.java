    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            if(privilegioOAB.equals(privRecepcionista) || privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                nombreCliente = request.getParameter("nombres");
                ...
                verificadorRegistro = cn.registrarCliente(nombreCliente, apellidoPaterno, apellidoMaterno, telefono, email, facebook);
                idCliente = 0;
                if(verificadorRegistro != false){
                    idCliente = cn.obtenerUltimoIDregistrado();
                }
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                if(privilegioOAB.equals(privRecepcionista) || privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                    if(verificadorRegistro != false) {
                        out.println("<h2>¡El cliente: \"" + nombreCliente + " "+ apellidoPaterno + " "+ apellidoMaterno + "\", se ha registrado exitosamente!</h2>");
                        out.println("<h3><a class='button fit' href='SelectServ?idCliente="+ idCliente + "'>Clic aquí para agendarle una cita</a></h3>");
                    }
                        else{
                            out.println("<h3>Nuevo cliente</h3>");
                            out.println("<h4 style='color: red;'>El cliente no ha podido registrarse, intente de nuevo. Si persiste el problema contacte al desarrollador de la aplicación</h4>");
                                out.println("<br/>");
                            out.println("<h3><a class='button fit' href='RegistrarCliente?nombres="+ nombreCliente + "&&apellidoPaterno=" + apellidoPaterno + "&&apellidoMaterno=" + apellidoMaterno +
                                    "&&telefono=" + telefono + "&&email=" + email + "&&facebook=" + facebook + "'>Clic aquí para intentar de nuevo</a></h3>");
                    }
                } else {
