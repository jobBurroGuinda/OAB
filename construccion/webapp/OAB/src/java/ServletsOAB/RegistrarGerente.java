 ...
 * @author Job
 */
public class RegistrarGerente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            if(privilegioOAB.equals(privAdmin)) {
                ...
                nombreGerente = request.getParameter("nombres");
                ...
                if(verificadorRegistro != false){
                    idGerente = cn.obtenerUltimoIDregistrado();
                }
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                if(privilegioOAB.equals(privAdmin)){
                    ...
                    if(verificadorExistenciaUN != true) {
                        verificadorRegistro = cn.registrarGerente(nomUsuario, password, nombreGerente, apellidoPaterno, apellidoMaterno, telefono, email, facebook);
                        // Si el registro se realizó correctamente...
                            if(verificadorRegistro != false) {
                                out.println("<h2>¡El gerente: \"" + nombreGerente + " "+ apellidoPaterno + " "+ apellidoMaterno + "\", se ha dado de alta exitosamente!</h2>");
                                out.println("<h3><a class='button fit' href='VerCitasdHoy'>Regresar a las citas de hoy</a></h3>");
                            }
                                else{
                                    out.println("<h3>Nuev@ gerente</h3>");
                                    out.println("<h4 style='color: red;'>La recepcionista no ha podido registrarse, intente de nuevo. Si persiste el problema contacte al desarrollador de la aplicación</h4>");
                                    out.println("<br/>");
