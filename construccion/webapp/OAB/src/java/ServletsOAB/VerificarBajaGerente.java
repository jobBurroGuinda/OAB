 ...
 * @author Job
 */
public class VerificarBajaGerente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null) {
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Baja de la recepcionista</title>");   
                ...
                out.println("<br/>");
                    ...
                    if(privilegioOAB.equals(privAdmin)){
                        gerentes = (List)sesion.getAttribute("DatosGerente");
                        tGerentes = gerentes.size();
                        for(int i=0 ; i<tGerentes ; i++) {
                            idUsuario = gerentes.get(i).getIdUsuario();
                            idGerente = gerentes.get(i).getIdPersona();
                            idContacto = gerentes.get(i).getIdContacto();
                        }   
                        verificador = cn.bajaRecepcionistaGerenteOAdmin(idUsuario, idContacto);
                        // Si la cita se cancela correctamente...
                        if(verificador != false) {
                            out.println("<h2 style='color:green;'>El gerente seleccionado se dio de baja exitosamente</h2>");
                            out.println("<a class='button fit special' href='VerCitasdHoy'>Regresar a las citas de hoy</a>");
                        }
                        // Si la cita no se pudo cancelar...
                        else {
                            out.println("<h2 style='color:red;'>El gerente no se pudo dar de baja. Intente de nuevo, si persiste el problema contácte al desarrollador de la aplicación.</h2>");
                            out.println("<table>");
                            for(int i=0 ; i<tGerentes ; i++){
                            ...
                            out.println("</table>");
                            out.println("   <a class='button fit special' href='VerificarBajaGerente?idGerente=" + idGerente + "'>Volver a intentar</a>");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(VerificarBajaGerente.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
