...
public class ActualizarAdministrador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            if(privilegioOAB.equals(privAdmin)   &&   verifIdentidadAdmin != false) {
                ...
                nomAdministrador = request.getParameter("nombres");
                ...
                facebook = request.getParameter("facebook");
                        administradores = (List)sesion.getAttribute("DatosAdmin");
                        tGerentes = administradores.size();
                for(int i=0 ; i<tGerentes ; i++) {
                    ...
                    verificadorAct = cn.actualizarAdmin(idUsuario, nomUsuario, password,
                                     idAdministrador, nomAdministrador, apellidoPaterno, apellidoMaterno, idContacto, telefono, email, facebook);
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Actualizar administrador</title>");
                ...
                if(privilegioOAB.equals(privAdmin)   &&   verifIdentidadAdmin != false) {
                    ...
                    if(verificadorAct != true) {
                                out.println("<h3>Actualizar administrador: '" + nomAdministrador + "'</h3>");
                                out.println("<h1 style='color:red;'>Los datos no se pudieron actualizar, intente de nuevo. Si persiste el problema contácte al desarrollador de la aplicación</h1>");
                                    out.println("<br/>");
                                out.println("<form method='POST' action='ActualizarAdministrador'>");
                                    ...
                                    out.println("</div>");
                                    out.println("<div class='row uniform 50%'>");
                                    out.println("   <input type='text' placeholder='Nombres *' name='nombres' id='nombres' value='" + nomAdministrador + "' required/>");
                                    out.println("</div>");
                                    out.println("<div class='row uniform 50%'>");
                    ...
                    // De lo contrario..., si los datos del cliente se actualizaron correctamente
                        ...
                        administradores = null;
                            administradores = cn.buscarAdmin_ID(idAdministrador);
                        sesion.setAttribute("DatosAdmin", administradores);
                            if(!administradores.isEmpty()) {
                                ...
                                out.println("<input type='text' id='nomAdministrador' name='nomAdministrador' placeholder='Introducir nombre' required>");
                                out.println("<input class='button special' value='Buscar' type='submit'>");
                                out.println("</form>");
                                out.println("<h1 style='color:green;'>Los datos del administrador se han actualizado exitosamente</h1>");
                                out.println("<a class='button fit special' href='EditarAdmin?idAdministrador=" + idAdministrador + "'>Editar</a>");
                                out.println("<table>");
                                for(int i=0 ; i<tGerentes ; i++){
                            ...
                            out.println("</table>");
                                out.println("<a class='button fit special' href='BajaAdministrador?idAdministrador=" + idAdministrador + "'>Dar de baja</a>");
                        } else {
                            out.println("<br/>");
                            out.println("<h1 style='color: red;'>¡Ha ocurrido un error!</h1>");
                            out.println("<h1>Puede que el administrador seleccionado haya sido dado de baja por otro usuario, verifique que el ID del ciente se encuentre en el link.</h1>");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(ActualizarAdministrador.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
