 ...
 * @author Job
 */
public class VerificarPosposicionCita extends HttpServlet {
        ...
        if(sesion != null) {
            ...
            if(privilegioOAB.equals(privRecepcionista) || privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)) {
                ...
                for(int i=0 ; i<tCita ; i++) {
                    idServicio = citas.get(i).getIdServicio();
                    idEspecialista = citas.get(i).getIdPersona();
                ...
                // Fin del encabezado
                    ...
                    if(privilegioOAB.equals(privRecepcionista) || privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                        ...
                        if(verifCitaHT != false  &&  verifCitaHC != false  &&  verifCitaD != false) {
                            ...
                            verificadorRC = cn.posponerCita(idCita, fecha, fHoraCita);
                        ...
                        if(verificadorRC != false) {
                                out.println("<h2>La cita se ha actualizado exitosamente</h2>");
                                for(int i=0 ; i<tCita ; i++) {
                                            nomCliente = citas.get(i).getNomCliente();
                                            out.println("<a class='button fit special' href='BusqClientepCita?nomCliente=" + nomCliente + "'>Nueva cita</a>");
                                    }
                                out.println("<a class='button fit special' href='VerCitasdHoy'>Regresar a las citas de hoy</a>");
                        }
                        else{
                            if(verifCitaD != true || verifCitaHC != true) {
                            ...
                            out.println("<form method='POST' action='VerificarPosposicionCita'>");
                                ...
                                out.println("<select name='idEspecialista' id='idEspecialista'>");
                                    for(int i=0 ; i<tCita ; i++) {
                                            nomEspecialista = citas.get(i).getNombre();
                                            out.println("<option value='" + citas.get(i).getIdPersona() + "'>Especialista: "+ nomEspecialista + "</option>");
                                    }
                                out.println("</select>");
                                out.println("<select name='idServicio' id='idServicio'>");
                                // Lista de servicios
                                    for(int i=0 ; i<tCita ; i++) {
                                            out.println("<option value='" + idServicio + "'>"+ "Servicio: "+ citas.get(i).getDescripcionServ() + "</option>");
                                    }
                                out.println("</select>");
                                out.println("<br/>");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(VerificarPosposicionCita.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
