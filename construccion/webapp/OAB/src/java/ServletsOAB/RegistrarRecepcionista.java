 ...
 * @author Job
 */
public class RegistrarRecepcionista extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            if(privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)) {
                ...
                nombreRecepcionista = request.getParameter("nombres");
                ...
                if(verificadorRegistro != false){
                    idRecepcionista = cn.obtenerUltimoIDregistrado();
                }
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Registrar recepcionista</title>");
                ...
                if(privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                    ...
                    if(verificadorExistenciaUN != true) {
                        verificadorRegistro = cn.registrarRecepcionista(nomUsuario, password, nombreRecepcionista, apellidoPaterno, apellidoMaterno, telefono, email, facebook);
                        // Si el registro se realizó correctamente...
                            if(verificadorRegistro != false) {
                                out.println("<h2>¡La recepcionista: \"" + nombreRecepcionista + " "+ apellidoPaterno + " "+ apellidoMaterno + "\", se ha dado de alta exitosamente!</h2>");
                                out.println("<h3><a class='button fit' href='VerCitasdHoy'>Regresar a las citas de hoy</a></h3>");
                                    ...
                                    out.println("<h4 style='color: red;'>La recepcionista no ha podido registrarse, puede que el nombre de usuario ya se encuentre ocupado<br> Intente de nuevo. Si persiste el problema contacte al desarrollador de la aplicación</h4>");
                                    out.println("<br/>");
                                    out.println("<form method='POST' name='formU' action='RegistrarRecepcionista'>");
                                        ...
                                        out.println("</div>");
                                        out.println("<div class='row uniform 50%'>");
                                        out.println("   <input type='text' placeholder='Nombres *' name='nombres' id='nombres' value='" + nombreRecepcionista + "' required/>");
                                        out.println("</div>");
                                        out.println("<div class='row uniform 50%'>");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(RegistrarRecepcionista.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
