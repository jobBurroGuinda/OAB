    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            if(privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                ...
                horaDuracion = Integer.parseInt(request.getParameter("horas"));
                minutosDuracion = Integer.parseInt(request.getParameter("minutos"));
                Servicio s = new Servicio();
                duracionServicio = s.obtenerDuracionFormatoHoras(horaDuracion, minutosDuracion);
                        servicios = (List)sesion.getAttribute("DatosServicio");
                        tServicios = servicios.size();
                for(int i=0 ; i<tServicios ; i++) {
                    idServicio = servicios.get(i).getIdServicio();
                }
                verificadorAct = cn.actualizarServicio(idServicio, servicio, duracionServicio);
            }
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Actualizar servicio</title>");
                ...
                if(privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)) {
                    ...
                    if(verificadorAct != true) {
                                out.println("<h3 style='color:red;'>Los datos no se pudieron actualizar, intente de nuevo. Si persiste el problema contácte al desarrollador de la aplicación</h3>");
                                    out.println("<br/>");
                    ...
                    // De lo contrario..., si los datos del cliente se actualizaron correctamente
                        ...
                        sesion.setAttribute("DatosServicio", servicios);
                            if(!servicios.isEmpty()) {
                                ...
                                out.println("<input type='text' id='nomServicio' name='nomServicio' placeholder='Introducir descripción' required>");
                                out.println("<input class='button special' value='Buscar' type='submit'>");
                                out.println("</form>");
                                out.println("<h3 style='color:green;'>¡Los datos del servicio se han actualizado exitosamente!</h3>");
                                out.println("<a class='button fit special' href='EditarServicio?idServicio=" + idServicio + "'>Editar</a>");
                                out.println("<table>");
                                    Servicio s = new Servicio();
                                ...
                                out.println("</table>");
                                out.println("<a class='button fit special' href='EliminarServicio?idServicio=" + idServicio + "'>Eliminar</a>");
                        } else {
                            out.println("<br/>");
