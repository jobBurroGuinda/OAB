 ...
 * @author Job
 */
public class BusqCitaspFecha extends HttpServlet {
                    ...
                    if(privilegioOAB.equals(privRecepcionista) || privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin))
                        {
                            if(request.getParameter("fechaCita") != null ){
                                fechaCita = request.getParameter("fechaCita");
                            }
                            citas = cn.verCitasPfecha(fechaCita);
                            int tCitas = citas.size();
                            if(tCitas > 0 ) {
                                    out.println("<h2>Buscar citas por fecha</h2>");
                                    out.println("<form method='POST' action='BusqCitaspFecha'>");
                                    out.println("<input type='date' value='" + fechaCita + "' min='" + fechaActual + "' name='fechaCita' id='fechaCita' required>");
                                    out.println("<input class='button special' value='Buscar' type='submit'>");
                                    out.println("</form>");
                                    out.println("<br/>");
                                    out.println("<h1 style='font-weight: bold;'>Citas programadas del día: '" + fechaCita + "'.</h1>");
                                ...
                                out.println("</table>");
                                out.println("<h3><a class='button fit' href='BusqCitaspFecha?fechaCita=" + fechaCita + "'>Actualizar</a></h3>");
                            } else{
                                out.println("<h2>Buscar citas por fecha</h2>");
                                    out.println("<form method='POST' action='BusqCitaspFecha'>");
                                    out.println("<input type='date' value='" + fechaCita + "' min='" + fechaActual + "' name='fechaCita' id='fechaCita' required>");
                                    ...
                                    out.println("</form>");
                                    out.println("<br/>");
                                    out.println("<h1 style='font-weight: bold;'>No se han agendado citas para el día: '" + fechaCita + "'.</h1>");
                            }      
                        }
                    // Si el usuario que inició sesión es un especialista...    
                    else if(privilegioOAB.equals(privEspecialista))
                        {
                            ...
                            citas = cn.verCitasPfechaYespecialista(fechaCita, username);
                            int tCitas = citas.size();
                            if(tCitas > 0 ) {
                            
                                    out.println("<h2>Buscar citas por fecha</h2>");
                                    out.println("<form method='POST' action='BusqCitaspFecha'>");
                                    out.println("<input type='date' value='" + fechaCita + "' min='" + fechaActual + "' name='fechaCita' id='fechaCita'>");
                                    out.println("<input class='button special' value='Buscar' type='submit'>");
                                    out.println("</form>");
                                    out.println("<br/>");
                                    out.println("<h1 style='font-weight: bold;'>Sus citas programadas para el día: '" + fechaCita + "'.</h1>");
                                ...
                                out.println("<h2>Buscar citas por fecha</h2>");
                                    ...
                                    out.println("</form>");
                                    out.println("<br/>");
                                    out.println("<h1 style='font-weight: bold;'>Aún no se le han agendado citas para el día: '" + fechaCita + "'.</h1>");
                            } 
                        }   
                    else {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(BusqCitaspFecha.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
