...
public class EditarAdmin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Editar datos del administrador</title>");
                ...
                if(privilegioOAB.equals(privAdmin) && verifIdentidadAdmin != false){
                    if(tAdministradores > 0) {
                        for(int i=0 ; i<tAdministradores ; i++) {
                            nomUsuario = administradores.get(i).getNomUsuario();
                            nomAdministrador = administradores.get(i).getNombre();
                            apellidoPaterno = administradores.get(i).getApellidoPaterno();
                            apellidoMaterno = administradores.get(i).getApellidoMaterno();
                            telefono = administradores.get(i).getTelefono();
                            email = administradores.get(i).getEmail();
                            facebook = administradores.get(i).getFacebook();
                            out.println("<h3>Modificar datos del administrador: '" + nomAdministrador + "'</h3>");
                                out.println("<br/>");
                    ...
                    // Momentos después de que este usuario haya consultado sus datos, en tal caso aparecerá este mensaje:
                        ...
                        out.println("<input type='text' id='nomAdministrador' name='nomAdministrador' autocomplete='on' placeholder='Introducir nombre' required>");
                    ...
                    out.println("<h2 style='color: orange;'>Sr. " + usernameOAB + ", usted no tiene permiso de modificar los datos de los administradores</h2>");
                } 
                else {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(EditarAdmin.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
