 ...
 * @author Job
 */
public class BusqCliente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sesion != null){
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Buscar citas por cliente</title>");
                ...
                if(privilegioOAB.equals(privRecepcionista) || privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                    ...
                    clientes = cn.buscarCliente(nomCliente);
                    sesion.setAttribute("DatosCliente", clientes);
                    int tClientes = clientes.size();
                    out.println("<h2>Buscar cliente</h2>");
                    out.println("<form method='POST' action='BusqCliente'>");
                    out.println("<input type='text' id='nomCliente' name='nomCliente' autocomplete='on' placeholder='Introducir nombre' value='"+nomCliente+"' required>");
                    ...
                    out.println("<table>");
                        ...
                        out.println("<tr>");
                            String nombre = p.obtenerNomCompleto(clientes.get(i).getNombre(), clientes.get(i).getApellidoPaterno(), clientes.get(i).getApellidoMaterno());
                            out.println("   <td>" + nombre + "</td>");
                            out.println("   <td><a class='button special small' href='DetallesCliente?idCliente=" + clientes.get(i).getIdPersona() + "'>Detalles</a></td>");
                            out.println("   <td><a class='button special small' href='EditarCliente?idCliente=" + clientes.get(i).getIdPersona() + "'>Editar</a></td>");
                            if(privilegioOAB.equals(privGerente) || privilegioOAB.equals(privAdmin)){
                                out.println("   <td><a class='button special small' href='BajaCliente?idCliente=" + clientes.get(i).getIdPersona() + "'>Baja</a></td>");
                            }
                        out.println("</tr>");
                        }
                        out.println("</table>");
                    } else{
                        out.println("<h2>El cliente \"" + nomCliente + "\", no se encuentra registrado</h2>");
                        out.println("<h3><a class='button fit' href='NvoCliente?nombreCliente="+ nomCliente +"'>Clic aquí para registrar</a></h3>");
                    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(BusqCliente.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
